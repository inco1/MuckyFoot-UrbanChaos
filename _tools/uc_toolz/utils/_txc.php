			...
			if($_POST['opt_txc_noFFFF'])
				...
				if($palType & 1) //paleta A4R4G4B4
					...
					for($j = $cNo; $j--; )
						...
						if(($colorsAlpha[] = $colorsNoAlpha[] = $colors[] = imageColorAllocateAlpha($im,$r,$g,$b,$a)) === false)
							{
...
/*	$gLimit = count($files)/64;
	...
	for($i = 0; $i <= $gLimit; $i++)
		...
		$grey = imageColorAllocate($paleta, 200, 200, 200);
		for($j = 0; $j <= 7; $j++)
			for($k = 0; $k <= 7; $k++)
				{
				set_time_limit(30);
				...
				if(file_exists($name))
					...
					imageString($paleta, 5, $x+67, $y, $i."_U".$j."_V".$k, $white);
					
					$gdzie = 4+4*($i*64+$k*8+$j);
					$string = "pos: 0x".dechex($gdzie);
					imageString($paleta, 5, $x+67, $y+12, $string, $grey);
					
					fSeek($plik, $gdzie);
					$string = "addr: 0x".dechex(fReadInt4($plik));
					imageString($paleta, 5, $x+67, $y+22, $string, $grey);
					
					if($i < 8)
						{
						$gdzie_ = $gdzie + 6276;
						fSeek($plik, $gdzie_);
						$string = "vmap pos: 0x".dechex($gdzie_);
						imageString($paleta, 5, $x+67, $y+34, $string, $white);
						
						$string = "vmap addr: 0x".dechex(fReadInt4($plik));
						imageString($paleta, 5, $x+67, $y+44, $string, $white);
						}
					}
					...
					fSeek($plik, $gdzie_);
//					$cos = ;
					$string = "?: ".dechex(ord(fRead($plik, 1)))." ".dechex(ord(fRead($plik, 1)));
					imageString($paleta, 5, $x+67, $y+56, $string, $grey);				
				}
